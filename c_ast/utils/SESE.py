...
class Graph:
    ...
    __slots__ = ('start', 'nodes', 'node_dic', 'num_edges',
                 'parent', 'dfs_num', 'finish_num', 'dfs_time',
    ...
    def undirected_dfs_visit(self, anod):
        self.dfs_time += 1
        self.dfs_num[anod] = self.dfs_time
        self.rev_dfs_num[self.dfs_time] = anod
        print("Visit", anod)
...
scenario in Johnson et al.'''
        ...
        for anod in reversed(self.post_order):
            print("For " + str(anod) + "\n  Brackets: {" +
                  ", ".join(str(brac) for brac in anod.blist.bracket_list)
                  + "}\n  back_edges: ["
                  + ", ".join([str(edg)
    ...
    def descendant(self, u, v):
        if (self.dfs_num[u] < self.dfs_num[v] and
           self.finish_num[u] > self.finish_num[v]):
            return True
        return False

    def ancestor(self, u, v):
        if (self.dfs_num[v] < self.dfs_num[u] and
