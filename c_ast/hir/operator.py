...
class Operator(object):
	...
	def __cmp__(self, other):
		if isinstance(other, Operator): 
			return cmp(self.to_str, other.to_str)
		else:
			return False
...
class BinaryOperators(object):
	...
	def __init__(self):
		self.binary_op = {
			...
			'SUBTRACT' : '-' } 
		self.operators = {}
		for k,v in self.binary_op.items():
			self.operators[k] = BinaryOperator(v)
			setattr(self, k, self.operators[k])
...
class UnaryOperators(object):
	...
	def __init__(self):
		self.unary_op = \
			...
			'PRE_INCREMENT' : '++'}
		self.operators = {}
		for k,v in self.unary_op.items():
...
class AssignmentOperators(object):
	...
	def __init__(self):
		self.assign_op = {'ADD' : '+=',
				...
				'SUBTRACT' : '-=' }
		self.operators = {}
		for k,v in self.assign_op.items():
